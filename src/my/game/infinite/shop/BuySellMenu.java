...
public abstract class BuySellMenu extends SlideEnterMenu {
    ...
    public BuySellMenu(GameState state, String asset, int price, Shop shop, InfinitePlayer player) {
        ...
        this.shop = shop;
        this.player = player;
        options.add(new SlidingOption(10, 10, 30, "Buy: " + price, LabelColor.GREEN, 0));
        options.add(new SlidingOption(10, 10, 30, "Sell: " + price / 2, LabelColor.GREEN, 1));
        options.add(new SlidingOption(400, 50, "Back", LabelColor.GREEN, 2));
        options.get(1).setX(630 - options.get(1).getWidth());
        ((SlidingLabel) options.get(1)).setFinalX(630 - options.get(1).getWidth());
        this.price = price;
        this.asset = new Label(10, 30, asset, LabelColor.GREEN);
        display = new Label(50, 30, "", LabelColor.GREEN);
    ...
    public void tick() {
        ...
        if (canBuy()) {
            ...
            if (price <= ((InfiniteState) state).getGame().getCredits()) {
                options.get(0).setColor(LabelColor.GREEN);
            } else {
                options.get(0).setColor(LabelColor.RED);
            }
        } else {
            options.get(0).setColor(LabelColor.GREY);
            options.get(0).setName("Maxed");
        }

        if (canSell()) {
            options.get(1).setColor(LabelColor.RED);
        } else {
            options.get(1).setColor(LabelColor.RED);
        }
    }
